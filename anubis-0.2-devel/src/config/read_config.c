...
int read_the_config_file(char *cfilename, struct _filenode *serverinfo) {
	FILE *inFile;
	...
	while(1) {
		...
		filenode->sfname = (char *)malloc(tempsize);
		...
		if((fread(&tempsize, sizeof(unsigned long), 1, inFile)) == 0)
			return -1;
		filenode->lfname = (char *)malloc(tempsize);
...
#endif
		if(serverinfo->serverkey == filenode->serverkey && 
			!strcmp(serverinfo->sfname,filenode->sfname) &&
			!strcmp(serverinfo->lfname,filenode->lfname) &&
			!strcmp(serverinfo->fingerprint,filenode->fingerprint)) 
				return 1;
